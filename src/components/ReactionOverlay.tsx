...
export default function ReactionOverlay() {
  ...
  // Announces each reaction exactly once, through a live region this
  // component owns itself — a VISIBLE toast for every emoji reaction (the
  // previous behaviour) was unwanted, so this no longer goes through the
  // shared toast store at all. The pointer-events-none caption below is
  // silent to a screen reader, so without this nobody using one would know a
  // reaction happened at all. Keyed by id (not array length): reactions
  // expire and are pruned independently, so a naive "new since last render"
